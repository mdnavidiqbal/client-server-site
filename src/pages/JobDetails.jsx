...
//                     {/* ✅ Link er poriborte button use koro */}
//                     <div className="mt-6">
//                         <button
//                             onClick={handleAccept}
//                             className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white py-2 px-4 rounded-lg font-semibold text-sm transition-all duration-200 transform hover:scale-105 shadow-md hover:shadow-lg"
//                         >
//                             Accept
//                         </button>
//                     </div>
...
const JobDetails = () => {
  ...
  const handleAccept = async (e) => {
    ...
    try {
      ...
      // 🧠 server থেকে আসা response JSON আকারে নিচ্ছি
      const result = await response.json()
      console.log(result)

      // ✅ Server যদি বলে already accepted
      if (response.status === 409 || result.message === 'This job is already accepted!') {
        toast.warning('You already accepted this job!', {
          position: 'top-right',
          autoClose: 3000,
        })
        return
      ...
      // ✅ Server যদি successfully insert করে
      if (result.success) {
        toast.success('Sucessfully Added Accepted Task', {
          position: 'top-right',
          autoClose: 3000,
        })
      } else {
  ...
  return (
    <div className="card shadow-lg bg-white rounded-xl overflow-hidden w-[450px] my-10 mx-auto">
      <div className="flex h-full">
        <div className="card-body w-full p-6 flex flex-col justify-between">
          <div>
            <h3 className="card-title text-2xl font-bold text-gray-900 mb-2">
