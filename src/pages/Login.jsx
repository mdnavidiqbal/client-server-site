...
export default function Login() {
  ...
  return (
    ...
    >
      ...
      {/* Login Card */}
      <div className="relative z-10 w-full max-w-sm sm:max-w-md md:max-w-md bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 sm:p-8 shadow-2xl text-white">
        {!loggedIn && (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <input
                ...
                onChange={(e) => setEmail(e.target.value)}
                placeholder="Email"
                className="input input-bordered w-full bg-white/20 placeholder-gray-200 text-white focus:outline-none focus:ring-2 focus:ring-purple-400"
              />
              <input
                value={password}
                onChange={(e) => setPassword(e.target.value)}
              ...
              <button className="btn btn-primary w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-semibold">
                Login
              </button>
              ...
              <button
                type="button"
                onClick={handleReset}
                className="link block text-center text-blue-300 hover:text-blue-400 text-sm"
              >
                Forgot password?
              </button>
            </form>
            ...
            <div className="mt-4">
              <button
                onClick={handleGoogle}
                className="btn btn-outline w-full text-white border-white hover:bg-white hover:text-black transition flex items-center justify-center gap-2"
              >
                <svg
                  aria-label="Google logo"
                  width="18"
        ...
        {loggedIn && (
          <animated.div
            style={animProps}
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-green-500 text-white p-6 rounded-lg shadow-lg w-72 sm:w-80 flex flex-col items-center"
          >
